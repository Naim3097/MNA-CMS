...
const fmtDate = (d) => {
  ...
  return date && !isNaN(date) ? date.toLocaleDateString('en-MY', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'
}
...
function EditPartModal({ part, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Error updating part: ${error.message || 'Please try again.'}`)
    } finally {
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      // Use the SAME compression pipeline as AddPartForm so we never store the
      // raw, uncompressed data URL (which can exceed Firestore's 1MB limit).
  ...
  const handleUrlChange = async (url) => {
    ...
    if (url && url.startsWith('http')) {
      try {
        ...
        // Keep the raw URL if it can't be read/compressed (e.g. CORS).
      }
    }
  }
    ...
    <ResponsiveModal
      ...
      footer={
        <>
          ...
          <button type="submit" form="edit-part-form" className="btn-primary" disabled={isSubmitting}>
    ...
    >
      <form id="edit-part-form" onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {isProcessingImage && <p className="field-hint text-ink mb-3">Processing image…</p>}
        ...
        <div className="card-flat">
          <div className="text-xs font-medium uppercase tracking-wide text-muted mb-2">Part history</div>
          <dl className="space-y-1 text-sm">
            <div className="flex justify-between gap-4">
              <dt className="text-muted">Date added</dt>
              <dd className="text-ink">{fmtDate(part.dateAdded)}</dd>
            </div>
            {part.dateUpdated && (
              <div className="flex justify-between gap-4">
                <dt className="text-muted">Last updated</dt>
                <dd className="text-ink">{fmtDate(part.dateUpdated)}</dd>
              </div>
            )}
          </dl>
        </div>
